...
function App() {
  ...
  const [isActivated, setIsActivated] = useState(null); // null = checking, false = locked, true = unlocked
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    setActiveTab('cashier'); // Reset tab for next login
  };
  ...
  useEffect(() => {
    if (lang === 'uz') {
      ...
      document.title = 'POS Магазин';
    }
  }, [lang, t]);

  if (isActivated === null) {
    return (
      <div className={`flex flex-col items-center justify-center h-screen font-sans ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
        <div className="flex flex-col items-center gap-4">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
          <p className="text-lg font-medium">Litsenziya tekshirilmoqda...</p>
        </div>
      </div>
    );
  }
  ...
  if (isActivated === false) {
    return (
      <div className={`flex items-center justify-center h-screen w-screen font-sans select-none transition-colors duration-300 ${theme === 'dark' ? 'dark bg-gray-950 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
        <div className="max-w-md w-full mx-4 bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-700/50 flex flex-col items-center transition-all duration-300 relative overflow-hidden">
          ...
          <div className="w-full bg-gray-50 dark:bg-gray-900/60 rounded-2xl p-4 mb-6 border border-gray-100 dark:border-gray-800/80">
            <span className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1">Kompyuter ID (Machine ID)</span>
            <code className="text-xs font-mono font-bold text-gray-700 dark:text-gray-300 break-all select-all">{currentMachineId || 'Aniqlanmoqda...'}</code>
          </div>

          {/* Input form */}
          <form onSubmit={handleActivationSubmit} className="w-full space-y-4">
            <div>
  ...
  return (
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'dark bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      {/* ── Sidebar ── */}
      <div className="w-64 bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 flex flex-col transition-colors duration-300">
        <div className="p-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 truncate">Кассир: {currentUser.name}</p>
            </div>
            <button
      ...
      {/* ── Main Content ── */}
      <div className={`flex-1 min-w-0 min-h-0 overflow-hidden transition-colors duration-300 ${activeTab === 'cashier' ? 'p-4' : 'p-6'}`}>
        {/* Cashier Tab */}
        <div style={{ display: activeTab === 'cashier' ? 'block' : 'none' }} className="h-full">
      ...
      {showShiftModal && (
        <ShiftModal 
          ...
          onShiftClosed={() => {
            // Optional: You could logout the user here if required.
            // handleLogout();
          }}
